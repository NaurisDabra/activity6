...
public class Point3DImpl extends Point2DImpl implements Point3D{
	...
	double d1=super.getDistance(p);
	double distance=Math.sqrt(Math.pow(d1, 2)+Math.pow(Math.abs(z-p.getZ()), 2));
		return distance;
	}
	...
	public double getDistance(double x2, double y2, double z2){
		double d1=super.getDistance(x2, y2);
		double distance=Math.sqrt(Math.pow(d1, 2)+Math.pow(Math.abs(z-z2), 2));
		return distance;
